...
{
    //研究实验基地：3-1 研究实验基地数1
    class ResearchInstitution3_1_1 : AbstractTable
    {
        public ResearchInstitution3_1_1 ()
        {
            dataCompile = new DataCompile();
            dataCompile.Caption = "3-1  研究实验基地数1";
            ...
            {
                if (baseBigType[i].IndexOf("实验室") != -1)
                {
            ...
            {
                HasError = true;
                Log.RecordLog(dataCompile.Caption + ": " + dataCompile.TableName + ": 找不到\"实验室\"列！");
                return;
            }
