...
{
    //科研院所和高校概况：1-2 科研院所和高校资产概况
    class ResearchInstitution1_2 : AbstractTable
    {
        public ResearchInstitution1_2()
        {
            tableCaption = "1-2 科研院所和高校资产概况";
            ...
            columnCheckList.AddRange(finalListA);
        ...
        public override bool GenerateSQL()
        {
            ...
            sqlStrForCompile = "select 所在省, convert(float, round(sum(cast(\"固定资产\" as float))/10000,2)) as \"固定资产\", convert(float, round(sum(cast(\"流动资产\" as float))/10000,2))  as \"流动资产\", convert(float, round(sum(cast(\"对外投资\" as float))/10000,2))  as \"对外投资\", convert(float, round(sum(cast(\"资产总额\" as float))/10000,2)) as '资产总额' from [" + dataTableName + "] where 所在省 is not null group by 所在省";
        ...
        public override void FillTable(List<Table> table)
        {
            ...
            newTable.Cell(1, 2).Range.Text = "资产总额";
            ...
            while (reader.Read())
            {
                ...
                //第二列，汇总列
                string value = reader["资产总额"].ToString().Trim();
                if (value.IndexOf('.') != -1)
                {
