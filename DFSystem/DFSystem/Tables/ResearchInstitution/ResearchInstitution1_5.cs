...
{
    //科研院所和高校概况：1-5 不同时期购置的科研院所和高校仪器设备数量
    class ResearchInstitution1_5 : AbstractTable
    {
        public ResearchInstitution1_5()
        {
            tableCaption = "1-5 不同时期购置的科研院所和高校仪器设备数量";
            dataTableName = SharedData.tableListInDB[1].ViewName;
            unitName = "台(套)";

            //用于检查列名
            columnCheckList.Add("2000年以前仪器数量合计");
            columnCheckList.Add("2001年-2005年仪器数量合计");
            columnCheckList.Add("2006年-2010年仪器数量合计");
            columnCheckList.Add("2011年仪器数量合计");
        ...
        public override bool GenerateSQL()
        {
            ...
            sqlStrForCompile = "select 所在省, convert(bigint, sum(cast(\"2000年以前仪器数量合计\" as float)))  as \"2000年以前\", convert(bigint, sum(cast(\"2001年-2005年仪器数量合计\" as float)))  as \"2001年-2005年\", convert(bigint, sum(cast(\"2006年-2010年仪器数量合计\" as float)))  as \"2006年-2010年\", convert(bigint, sum(cast(\"2011年仪器数量合计\" as float)))  as \"2011年\", ( convert(bigint, sum(cast(\"2000年以前仪器数量合计\" as float)))  + convert(bigint, sum(cast(\"2001年-2005年仪器数量合计\" as float)))  + convert(bigint, sum(cast(\"2006年-2010年仪器数量合计\" as float)))  + convert(bigint, sum(cast(\"2011年仪器数量合计\" as float)))  ) as '合计' FROM ["
