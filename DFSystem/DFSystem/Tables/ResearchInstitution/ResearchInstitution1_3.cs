...
{
    //科研院所和高校概况：1-3 按单台原值分段的科研院所和高校仪器设备数量
    class ResearchInstitution1_3 : AbstractTable
    {
        public ResearchInstitution1_3()
        {
            tableCaption = "1-3 按单台原值分段的科研院所和高校仪器设备数量";
            dataTableName = SharedData.tableListInDB[1].ViewName;
            unitName = "台（套）";

            finalListA.Add("20万元以下");
            finalListA.Add("20-50万元");
            finalListA.Add("50万元以上");

            columnCheckList.Add("原值≤20仪器数量合计");
            columnCheckList.Add("50﹥原值≥20仪器数量合计");
            columnCheckList.Add("原值≥50仪器数量合计");
        ...
        public override bool GenerateSQL()
        {
            ...
            sqlStrForCompile = "select 所在省, convert(float, sum(cast(\"原值≤20仪器数量合计\" as float)))  as \"20万元以下\", convert(float, sum(cast(\"50﹥原值≥20仪器数量合计\" as float)))  as \"20-50万元\", convert(float, sum(cast(\"原值≥50仪器数量合计\" as float)))  as \"50万元以上\", ( convert(float, sum(cast(\"原值≤20仪器数量合计\" as float)))  + convert(float, sum(cast(\"50﹥原值≥20仪器数量合计\" as float)))  + convert(float, sum(cast(\"原值≥50仪器数量合计\" as float)))  ) as '合计' from [" + dataTableName + "] group by 所在省";
