...
{
    //科研院所和高校概况：1-10 科研院所和高校人才培养情况3
    class ResearchInstitution1_10_3 : AbstractTable
    {
        public ResearchInstitution1_10_3()
        {
            ...
            //用于检查列名
            columnCheckList.Add("博士后入学（入站）数");
            columnCheckList.Add("博士后毕业（出站）数");
            columnCheckList.Add("博士后在读（在站）数");
            columnCheckList.Add("博士后合计");
        ...
        public override bool GenerateSQL()
        {
            ...
            fileName = tableCaption + "3";

            sqlStrForCompile = "select 所在省, convert(bigint, sum(cast(\"博士后入学（入站）数\" as float)))  as \"入学\", convert(bigint, sum(cast(\"博士后毕业（出站）数\" as float)))  as \"毕业\", convert(bigint, sum(cast(\"博士后在读（在站）数\" as float)))  as \"在读\", ( convert(bigint, sum(cast(\"博士后合计\" as float)))) as '博士后' FROM ["
        ...
        public override void FillTable(List<Table> table)
        {
            ...
            newTable.Cell(1, 2).Range.Text = "博士后";
            ...
            while (reader.Read())
            {
                ...
                //第二列，汇总列
                string value = reader["博士后"].ToString().Trim();
                if (value.IndexOf('.') != -1)
                {
