...
{
    //科研院所和高校概况：1-10 科研院所和高校人才培养情况1
    class ResearchInstitution1_10_1 : AbstractTable
    {
        public ResearchInstitution1_10_1()
        {
            ...
            //用于检查列名
            columnCheckList.Add("硕士研究生入学（入站）数");
            columnCheckList.Add("硕士研究生毕业（出站）数");
            columnCheckList.Add("硕士研究生在读（在站）数");
            columnCheckList.Add("硕士研究生合计");
        ...
        public override bool GenerateSQL()
        {
            ...
            fileName = tableCaption + "1";

            sqlStrForCompile = "select 所在省, convert(bigint, sum(cast(\"硕士研究生入学（入站）数\" as float)))  as \"入学\", convert(bigint, sum(cast(\"硕士研究生毕业（出站）数\" as float)))  as \"毕业\", convert(bigint, sum(cast(\"硕士研究生在读（在站）数\" as float)))  as \"在读\", ( convert(bigint, sum(cast(\"硕士研究生合计\" as float)))) as '硕士研究生' FROM ["
        ...
        public override void FillTable(List<Table> table)
        {
            ...
            newTable.Cell(1, 2).Range.Text = "硕士研究生";
            ...
            while (reader.Read())
            {
                ...
                //第二列，汇总列
                string value = reader["硕士研究生"].ToString().Trim();
                if (value.IndexOf('.') != -1)
                {
