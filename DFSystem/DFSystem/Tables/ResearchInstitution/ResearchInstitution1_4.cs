...
{
    //科研院所和高校概况：1-4 按单台原值分段的科研院所和高校仪器设备原值
    class ResearchInstitution1_4 : AbstractTable
    {
        public ResearchInstitution1_4()
        {
            tableCaption = "1-4 按单台原值分段的科研院所和高校仪器设备原值";
            ...
            //用于检查列名
            columnCheckList.Add("原值≤20合计");
            columnCheckList.Add("50﹥原值≥20合计");
            columnCheckList.Add("原值≥50合计");
        ...
        public override bool GenerateSQL()
        {
            ...
            sqlStrForCompile = "select 所在省, convert(float, sum(cast(\"原值≤20合计\" as float)) / 10000)  as \"20万元以下\", convert(float, sum(cast(\"50﹥原值≥20合计\" as float))/ 10000)  as \"20-50万元\", convert(float, sum(cast(\"原值≥50合计\" as float))/ 10000)  as \"50万元以上\", ( convert(float, sum(cast(\"原值≤20合计\" as float)))  + convert(float, sum(cast(\"50﹥原值≥20合计\" as float)))  + convert(float, sum(cast(\"原值≥50合计\" as float))))/ 10000 as '合计' FROM ["
