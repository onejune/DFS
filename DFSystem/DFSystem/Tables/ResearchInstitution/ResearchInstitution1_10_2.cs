...
{
    //科研院所和高校概况：1-10 科研院所和高校人才培养情况2
    class ResearchInstitution1_10_2 : AbstractTable
    {
        public ResearchInstitution1_10_2()
        {
            tableCaption = "1-10 科研院所和高校人才培养情况";
            dataTableName = SharedData.tableListInDB[2].ViewName;
            unitName = "人";

            //用于检查列名
            columnCheckList.Add("博士研究生入学（入站）数");
            columnCheckList.Add("博士研究生毕业（出站）数");
            columnCheckList.Add("博士研究生在读（在站）数");
            columnCheckList.Add("博士研究生合计");
        ...
        public override bool GenerateSQL()
        {
            ...
            fileName = tableCaption + "2";

            sqlStrForCompile = "select 所在省, convert(bigint, sum(cast(\"博士研究生入学（入站）数\" as float)))  as \"入学\", convert(bigint, sum(cast(\"博士研究生毕业（出站）数\" as float)))  as \"毕业\", convert(bigint, sum(cast(\"博士研究生在读（在站）数\" as float)))  as \"在读\", ( convert(bigint, sum(cast(\"博士研究生合计\" as float)))) as '博士研究生' FROM ["
        ...
        public override void FillTable(List<Table> table)
        {
            ...
            newTable.Cell(1, 2).Range.Text = "博士研究生";
            ...
            while (reader.Read())
            {
                ...
                //第二列，汇总列
                string value = reader["博士研究生"].ToString().Trim();
                if (value.IndexOf('.') != -1)
                {
